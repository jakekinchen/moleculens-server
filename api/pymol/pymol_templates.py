...
def _rcsb_find_first_id(query: str) -> str | None:
    ...
    url = "https://search.rcsb.org/rcsbsearch/v2/query?json=" + urllib.parse.quote(json.dumps(payload))
    try:
...
def _get_structure_load_command(structure_id: str) -> str:
    ...
    try:
        ...
        search_results = client.search_service.search_by_name_direct(structure_id)
        ...
        sdf_text = client.fetch_sdf_for_cid(cid)
        if not sdf_text:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".sdf") as fp:
            ...
            tmp_path = fp.name
        threading.Timer(2, lambda p: os.unlink(p) if os.path.exists(p) else None, args=[tmp_path]).start()
...
def annotated_molecule_scene(structure_id: str, annotations: list[dict] | None = None) -> list[str]:
    ...
    if annotations:
        for annotation in annotations:
            if annotation["type"] == "distance":
                commands.append(f"distance {annotation['name']}, {annotation['atom1']}, {annotation['atom2']}")
            elif annotation["type"] == "label":
                commands.append(f"label {annotation['selection']}, '{annotation['text']}'")
...
def transparent_binding_site_scene(structure_id: str, selection: str, transparent_bg: bool = True) -> list[str]:
