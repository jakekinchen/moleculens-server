...
""",
    ...
    async def generate_geometry_from_plan(
        ...
        print(
            f"Starting to generate geometry for {len(plan.objects)} objects in parallel..."
        )
        ...
        async def process_object(i, obj):
            ...
            try:
                ...
                failures.append({"object_name": obj.name, "error": str(e)})
                return obj.name, {
                    "object_info": obj.dict(),
                    "status": "failed",
                    "error": str(e),
    ...
    def _format_object_prompt(self, obj: SceneObject, scene_title: str) -> str:
        ...
        relationships_str = (
            ", ".join(obj.relationships) if obj.relationships else "None"
        )
