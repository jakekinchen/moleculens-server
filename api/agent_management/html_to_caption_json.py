...
def extract_captions_from_html(html_content):
    ...
    captions_match = re.search(r'const\s+captions\s*=\s*\[(.*?)\];', script_content, re.DOTALL)
    ...
    for match in re.finditer(r'{[^}]*}', captions_text):
        ...
        if time_match and text_match:
            ...
            timecode = f"{int(time_obj.total_seconds() // 3600):02d}:" \
...
def convert_html_to_json(input_data, output_file_path=None):
    ...
    try:
        ...
        if isinstance(input_data, dict) and 'html' in input_data:
            ...
            raise ValueError("Invalid input format. Expected file path, HTML string, or JSON object with 'html' key")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python html_to_caption_json.py <input_html_file_or_json> [output_json_file]")
        sys.exit(1)
