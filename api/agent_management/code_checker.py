...
class CodeQualityTool:
    ...
    async def process_code(
        ...
        visual_result = await self._check_visual_outcome(
            build_result["screenshots"], visual_outcome
        )
    ...
    async def submit_diff_and_check(
        ...
        return await self.process_code(
            updated_code, language, code_type, visual_outcome, timecodes
        )
    ...
    async def _run_threejs_component(
        self, code: str, timecodes: Optional[List[int]] = None
    ) -> Dict:
    ...
    async def _run_html_standalone(
        self, code: str, timecodes: Optional[List[int]] = None
    ) -> Dict:
    ...
    async def _run_html_in_playwright(
        ...
        async with async_playwright() as p:
            try:
                    ...
                    if not timecodes:
                        ...
                        for tc in sorted(timecodes):
                            await page.wait_for_timeout(
                                tc
                                if tc == timecodes[0]
                                else tc - timecodes[timecodes.index(tc) - 1]
                            )
                            screenshot_path = os.path.join(
                                temp_dir, f"render_{tc}ms.png"
                            )
                            await page.screenshot(path=screenshot_path)
                            screenshots.append(screenshot_path)
                ...
                if browser:
                    await browser.close()
                return {"success": False, "error": str(e), "screenshots": screenshots}
    ...
    async def _run_python_code(
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                return {"success": False, "error": result.stderr, "screenshots": []}
