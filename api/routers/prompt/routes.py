...
from agent_management.model_config import ModelInfo, ModelCategory, get_default_model
...
from agent_management.llm_service import LLMService, LLMModelConfig, ProviderType, StructuredLLMRequest
...
# Diagram Feature Pydantic Models - Moved to absolute top
...
class Arrow(BaseModel):
    """Represents an arrow in the diagram."""
    start: List[float]
    end: List[float]
...
class DiagramPromptRequest(BaseModel):
    """Request model for generating a molecule diagram."""
    prompt: str
...
class DiagramPlan(BaseModel):
    """Plan for the molecule diagram, typically generated by an LLM."""
    plan: str
...
class DiagramResponse(BaseModel):
    """Response model for a generated molecule diagram."""
    diagram_image: str
    diagram_plan: dict
    status: Literal["completed", "failed", "processing"] = "completed"
    job_id: Optional[str] = None
    error: Optional[str] = None
# End of Diagram Feature Pydantic Models
...
async def generate_molecule_diagram(request: DiagramPromptRequest, llm_service: LLMService = Depends(use_llm)) -> DiagramResponse:
    """
    ...
    try:
        model_info_instance = ModelRegistry.create_instance(request.model or get_default_model())
        ...
        formatted_system_prompt = DIAGRAM_SYSTEM_PROMPT_TEMPLATE.replace(
            ...
            "{{canvas_height}}", str(request.canvas_height)
        )
